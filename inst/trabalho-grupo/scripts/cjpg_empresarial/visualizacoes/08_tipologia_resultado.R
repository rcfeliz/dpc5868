...
base_extinto <- base_raw |>
  dplyr::mutate(classe = dplyr::if_else(resultado == "Extinção", "Extinto", "Não extinto"))

base_resultado <- base_raw |>
  dplyr::filter(resultado != "Extinção") |>
  dplyr::mutate(
    classe = dplyr::case_when(
      resultado %in% c("Parcialmente procedente", "Totalmente procedente") ~ "Procedente",
      TRUE ~ "Improcedente"
    )
  )
...
ft_extinto <- base_extinto |>
  ...
  dplyr::mutate(tipologia = dplyr::if_else(!duplicated(tipologia), as.character(tipologia), NA_character_)) |>
  flextable::flextable() |>
  flextable::set_header_labels(tipologia = "Tipologia", classe = "Resultado", n = "N", prop = "%") |>
