...
jucesp_download <- function(cnpj, pasta) {
  ...
  viewstate <- rvest::html_element(html_get, "#__VIEWSTATE") |>
    rvest::html_attr("value")
  eventval <- rvest::html_element(html_get, "#__EVENTVALIDATION") |>
    rvest::html_attr("value")
  viewstate_gen <- rvest::html_element(html_get, "#__VIEWSTATEGENERATOR") |>
    rvest::html_attr("value")

  message("[jucesp] ", cnpj_limpo, " | viewstate ok, fazendo busca")

  html_busca <- jucesp_buscar(
    cnpj_limpo,
    viewstate,
    eventval,
    viewstate_gen,
  ...
  captcha_panel <- rvest::html_element(
    html_busca,
    "#ctl00_cphContent_gdvResultadoBusca_pnlCaptcha"
  )
  if (!is.na(captcha_panel)) {
    message("[jucesp] ", cnpj_limpo, " | captcha detectado, resolvendo")
    html_busca <- jucesp_resolver_captcha(
      cnpj_limpo,
      html_busca,
      cookie_file,
      pasta
    ...
    message(
      "[jucesp] ",
      cnpj_limpo,
      " | captcha resolvido, html de busca retornado"
    )
  } else {
  ...
  message(
    "[jucesp] ",
    cnpj_limpo,
    " | buscando ficha cadastral (nire=",
    trimws(nire),
    ")"
  )

  resp_detalhe <- httr2::request(
    "https://www.jucesponline.sp.gov.br/Pre_Visualiza.aspx"
  ) |>
...
jucesp_resolver_captcha <- function(
  ...
  jucesp_submeter_captcha <- function(html_ref, codigo) {
    vs <- rvest::html_element(html_ref, "#__VIEWSTATE") |>
      rvest::html_attr("value")
    evd <- rvest::html_element(html_ref, "#__EVENTVALIDATION") |>
      rvest::html_attr("value")
    vsg <- rvest::html_element(html_ref, "#__VIEWSTATEGENERATOR") |>
      rvest::html_attr("value")
    httr2::request("https://www.jucesponline.sp.gov.br/ResultadoBusca.aspx") |>
      httr2::req_cookie_preserve(cookie_file) |>
      httr2::req_body_form(
  ...
  codigo <- jucesp_ler_captcha(img_path)
  html_resultado <- jucesp_submeter_captcha(html_busca, codigo)
  unlink(img_path)
  html_resultado
}
...
jucesp_parse <- function(arquivo) {
  ...
  campo <- function(id) {
    rvest::html_element(
      html,
      paste0("#ctl00_cphContent_frmPreVisualiza_", id)
    ) |>
      rvest::html_text2() |>
      trimws()
  }
